...
### **데이터 처리 개선**
- **중복 토큰 자동 처리**: 같은 단어가 여러 번 등장해도 오류 없음
- **고유 인덱스 생성**: `word_0`, `word_1` 형태로 자동 구분
- **안정적인 DataFrame 생성**: pandas와 pyarrow 호환성 보장
...
영어: "I ate a meal"
```
- **Self-Attention**: 한국어 문장 내 단어 간 관계 학습
- **Cross-Attention**: 영어 단어가 한국어 단어를 어떻게 참고하는지 확인
...
```
- **중복 토큰 처리**: "you"가 두 번 등장해도 자동으로 구분
- **긴 문장 분석**: 복잡한 어텐션 패턴 시각화

### **예시 3: 멀티헤드 어텐션**
- **4개 헤드**: 각각 다른 관점에서 어텐션 패턴 학습
- **병렬 처리**: 동시에 다양한 관계 파악
- **결합**: 모든 헤드의 결과를 종합하여 최종 출력
...
- **중복 토큰**: 같은 단어가 여러 번 등장하면 자동으로 구분됨

## 🐛 문제 해결

### **자주 발생하는 문제들**
1. **한글 깨짐**: 시스템별 폰트 자동 선택으로 해결
2. **중복 컬럼 오류**: `np_to_df` 함수가 자동으로 처리
3. **폰트 경고**: matplotlib 설정으로 한글 폰트 문제 해결

### **성능 최적화**
- **Watchdog 모듈**: 파일 변경 감지 성능 향상
- **가상환경**: 의존성 충돌 방지
...
### **기여 방법**
1. **Issue 등록**: 버그 리포트 또는 기능 제안
2. **Pull Request**: 코드 개선 또는 새로운 기능 추가
3. **문서 개선**: README, 주석, 설명서 업데이트
